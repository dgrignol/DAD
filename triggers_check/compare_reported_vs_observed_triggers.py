...
aligns the reported and observed trigger sequences, and writes:
...
- Short transition pulses from digital line updates (e.g., 0->A->B->0 with
  a 1-sample A) are always collapsed before alignment, counted as WARNINGS,
  and plotted in +/-window diagnostics unless explicitly disabled.
...
- transition warning plot PNGs (one per warning, default window +/-3 ms)
...
4) Custom anomaly window (+/-750 ms) and explicit plot directory
   python compare_reported_vs_observed_triggers.py \
     --reported-csv /path/debug.csv \
     --fif /path/data.fif \
     --out-csv /tmp/confrontation.csv \
...
5) Open interactive scroller after writing outputs
   python compare_reported_vs_observed_triggers.py \
     --reported-csv /path/debug.csv \
     --fif /path/data.fif \
     --out-csv /tmp/confrontation.csv \
...
6) Disable transition-pulse warning plots (warnings still reported)
   python compare_reported_vs_observed_triggers.py \
     --subject 52 \
     --block 1 \
     --run 1 \
...
class TransitionPulseWarning:
    """A collapsed short transition pulse kept as a reportable warning."""
...
def _parse_optional_float(value: str | None) -> Optional[float]:
    """Parse a float-like cell, treating empty values as missing."""
    if value is None:
        return None
    value = value.strip()
    if not value:
...
def collapse_transition_windows(
    ...
    *,
    ...
    """
    Remove short transition pulses in contiguous non-zero transitions.

    Hardware line updates can produce patterns like:
      0 -> A -> B -> 0
    where A is a 1-sample transient while B is the intended trigger code.

    Collapsing rule:
    - if window_i width <= transient_max_samples
    - and window_i ends exactly where window_{i+1} starts
    then window_i is dropped as a transition artifact.
    """
    ...
    for idx in range(len(windows) - 1):
        ...
        nxt = windows[idx + 1]
        if (
            cur.width_samples <= transient_max_samples
            and cur.end_sample == nxt.start_sample
        ):
            keep[idx] = False
            transition_warnings.append(
                TransitionPulseWarning(
                    warning_idx_1based=len(transition_warnings) + 1,
    ...
    return collapsed, transition_warnings
...
def _select_reported_anchor_for_transition_warning(
    *,
    reported_anchors: list[ReportedAnchor],
    ...
    next_trigger: int,
    anchor_s: float,
) -> tuple[Optional[ReportedAnchor], str]:
    """
    Select one reported-anchor reference for a transition-warning plot.

    Selection rule
    - Search only finite anchors inside the plotted time window.
    - Prefer anchors whose reported trigger matches dropped/next trigger.
    - Break ties by nearest time to the dropped-step start.
    """
    local_anchors = [
        a
        for a in reported_anchors
        if np.isfinite(a.anchor_s) and window_start_s <= a.anchor_s <= window_end_s
    ]
    if not local_anchors:
        return None, "no_reported_anchor_in_window"

    preferred = [
        a for a in local_anchors if a.reported_trigger in (dropped_trigger, next_trigger)
    ]
    candidates = preferred if preferred else local_anchors
    selected = min(candidates, key=lambda a: abs(a.anchor_s - anchor_s))

    if selected.reported_trigger == dropped_trigger:
        relation = "reported_matches_dropped_step"
    elif selected.reported_trigger == next_trigger:
        relation = "reported_matches_next_step"
    else:
        relation = "reported_matches_neither_step"
...
def plot_transition_warning_windows(
    *,
    ...
    """
    Save one trigger-channel plot per collapsed transition pulse warning.

    Each plot covers ±window_ms around the dropped transition pulse onset and
    includes:
    - raw observed trigger channel signal,
    - highlighted dropped/next step boundaries,
    - orange shaded interval for the dropped transient pulse,
    - a selected reported-anchor reference (green),
    - faded vertical lines for other reported-trigger anchors in the window.
    """
    if not warnings:
        return
    ...
    for warning in warnings:
        anchor_sample = warning.dropped_start_sample
        start_sample = max(abs_min_sample, anchor_sample - win_samples)
        end_sample = min(abs_max_sample, anchor_sample + win_samples)
        if end_sample <= start_sample:
            continue
        ...
        window_start_s = start_sample / sfreq
        window_end_s = end_sample / sfreq
        selected_anchor, relation_key = _select_reported_anchor_for_transition_warning(
            reported_anchors=reported_anchors,
            window_start_s=window_start_s,
            window_end_s=window_end_s,
            dropped_trigger=warning.dropped_trigger,
            next_trigger=warning.next_trigger,
            anchor_s=warning.dropped_start_s,
        )
        relation_text_map = {
            "reported_matches_dropped_step": "reported matches DROPPED step",
            "reported_matches_next_step": "reported matches NEXT step",
            "reported_matches_neither_step": "reported matches neither dropped nor next step",
            "no_reported_anchor_in_window": "no reported anchor in this window",
        }
        relation_text = relation_text_map.get(relation_key, relation_key)

        # Emphasize the collapsed transient and its boundary to the next trigger.
        ax.axvline(
            warning.dropped_start_s,
            color="#ff7f0e",
            linestyle="--",
            linewidth=1.3,
        )
        ax.axvline(
            warning.next_start_s,
            color="#d62728",
            linestyle=":",
            linewidth=1.1,
        )
        ax.axvspan(
            warning.dropped_start_s,
            warning.dropped_end_s,
            color="#ff7f0e",
            alpha=0.18,
        )

        # Add faded lines for other reported anchors in this local window.
        for rep_anchor in reported_anchors:
            if not np.isfinite(rep_anchor.anchor_s):
                continue
            if window_start_s <= rep_anchor.anchor_s <= window_end_s:
                if selected_anchor is not None and rep_anchor is selected_anchor:
                    continue
                ax.axvline(rep_anchor.anchor_s, color="#555555", alpha=0.16, linewidth=0.8)

        # Explicitly mark one reported reference so observed-vs-reported is obvious.
        if selected_anchor is not None:
            ax.axvline(
                selected_anchor.anchor_s,
                color="#2ca02c",
                linestyle="-",
                linewidth=1.4,
                zorder=4,
        ...
        # Annotate nearby observed trigger onsets for context.
        for obs in observed:
            if start_sample <= obs.start_sample <= end_sample:
                ax.axvline(obs.start_s, color="#bbbbbb", linewidth=0.7, alpha=0.8)
                ax.text(
        ...
        # Short in-plot explanation: reported reference and color semantics.
        if selected_anchor is not None:
            ref_text = (
                f"Reported ref: trig={selected_anchor.reported_trigger}"
                f" ({selected_anchor.reported_label or 'no_label'}) "
                f"at {selected_anchor.anchor_s:.6f}s -> {relation_text}"
            )
        else:
            ref_text = "Reported ref: none in this window"
        ax.text(
            0.01,
            0.99,
            ref_text,
        ...
        legend_handles = [
            Line2D([0], [0], color="#1f77b4", linewidth=1.1, label="Observed trigger channel"),
            Line2D(
                [0],
                [0],
                color="#ff7f0e",
                linestyle="--",
                linewidth=1.3,
                label=f"Dropped step start ({warning.dropped_trigger})",
            ),
            Line2D(
                [0],
                [0],
                ...
                linestyle=":",
                linewidth=1.1,
                label=f"Next step start ({warning.next_trigger})",
            ),
            Patch(
                facecolor="#ff7f0e",
                alpha=0.18,
                edgecolor="none",
                label="Orange region: dropped transient pulse (collapsed)",
            ),
            Line2D(
                [0],
                [0],
        ...
        if selected_anchor is not None:
            legend_handles.append(
                Line2D(
                    ...
                    linewidth=1.4,
                    label=(
                        f"Selected reported ref ({selected_anchor.reported_trigger}) "
                        f"- {relation_text}"
                    ),
                )
            )
        ...
        ax.set_title(
            ...
            f"dropped={warning.dropped_trigger} "
            f"next={warning.next_trigger} | "
            f"start={warning.dropped_start_s:.3f}s"
        )
        ax.grid(True, alpha=0.25)
        ax.set_xlim(window_start_s, window_end_s)
        ax.legend(handles=legend_handles, loc="lower left", fontsize=6.7, framealpha=0.9)
        fig.tight_layout()

        filename = (
            f"{warning.warning_idx_1based:03d}_transition_warning_"
        ...
        warning.window_start_s = window_start_s
        warning.window_end_s = window_end_s
        ...
        rows.append(
            {
                ...
                "reported_seconds": (
                    "" if (rep is None or rep.seconds is None) else f"{rep.seconds:.6f}"
                ),
...
def _path_for_report(path: Path, target: str) -> str:
    """Format target path relative to the markdown report directory when possible."""
    if not target:
        return ""
    ...
    try:
        return Path(os.path.relpath(target_path, start=path.parent)).as_posix()
    except Exception:
        return target_path.as_posix()
...
def write_markdown_report(
    ...
    *,
    ...
    reported_csv_disp = _path_for_report(path, str(reported_csv))
    fif_path_disp = _path_for_report(path, str(fif_path))
    out_csv_disp = _path_for_report(path, str(out_csv))
    anomaly_csv_disp = _path_for_report(path, str(anomaly_csv))
    anomaly_plots_dir_disp = _path_for_report(path, str(anomaly_plots_dir))
    transition_plots_dir_disp = _path_for_report(path, str(transition_plots_dir))

    lines: list[str] = []
    lines.append("# Trigger Confrontation Report")
    lines.append("")
    lines.append("## Inputs")
    lines.append(f"- Reported CSV: `{reported_csv_disp}`")
    lines.append(f"- FIF file: `{fif_path_disp}`")
    ...
    lines.append("")
    lines.append("| category | count | explanation |")
    lines.append("| --- | --- | --- |")
    lines.append(
        "| WARNINGS | "
        f"{len(transition_warnings)} | "
        "Collapsed short contiguous transition pulses (0->A->B->0 line-update transients) before alignment. |"
    )
    lines.append("")
    ...
    lines.append(f"- Full alignment CSV: `{out_csv_disp}`")
    lines.append(f"- Anomaly CSV: `{anomaly_csv_disp}`")
    lines.append(f"- Anomaly plots directory: `{anomaly_plots_dir_disp}`")
    lines.append(f"- Transition warning plots directory: `{transition_plots_dir_disp}`")
    lines.append("")
    ...
    lines.append(f"- Trigger window around each anomaly: ±{anomaly_plot_window_ms:.1f} ms.")
    if not anomalies:
    ...
    lines.append("")
    lines.append("## Transition Pulse Warnings")
    lines.append(f"- Window around each warning: ±{transition_plot_window_ms:.1f} ms.")
    lines.append(
        "- In each plot, the orange shaded region is the dropped transient pulse "
        "interval that was collapsed before alignment."
    )
    if transition_plots_disabled:
        lines.append("- Transition pulse plotting was disabled with `--disable-plot-transition-pulses`.")
    if not transition_warnings:
        lines.append("No short contiguous transition pulses were collapsed.")
    else:
        lines.append(
            "| warning_idx | dropped_trigger | next_trigger | dropped_start_s | "
            "dropped_width_samples | detail | plot_path |"
        )
        lines.append("| --- | --- | --- | --- | --- | --- | --- |")
        for warning in transition_warnings:
            lines.append(
                ...
                + " | ".join(
                    [
                        str(warning.warning_idx_1based),
                        str(warning.dropped_trigger),
                        str(warning.next_trigger),
                        f"{warning.dropped_start_s:.6f}",
                        str(warning.dropped_width_samples),
                        _md_escape(warning.detail),
                        _md_escape(_path_for_report(path, warning.plot_path)),
                    ]
                )
                + " |"
            )
        lines.append("")
        if transition_plots_disabled:
            lines.append("Transition warning plots were not generated.")
        else:
            for warning in transition_warnings:
                if not warning.plot_path:
                    continue
                lines.append(
                    f"### Transition Warning {warning.warning_idx_1based}: "
                    f"{warning.dropped_trigger}->{warning.next_trigger}"
                )
                lines.append(f"- Detail: {warning.detail}")
                lines.append(
                    f"![Transition Warning {warning.warning_idx_1based}]"
                    f"({_path_for_report(path, warning.plot_path)})"
                )
                lines.append("")
    ...
    lines.append(
        ...
        "reported_frame | reported_seconds | reported_label | observed_idx | observed_trigger | "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "Project root containing data/, experiment/output_files/, and reports/ "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to debug trigger CSV. If omitted, resolved from "
            "subject/block/run in experiment/output_files."
        ),
    )
    parser.add_argument(
        "--fif",
        ...
        default=None,
        help=(
            "Path to FIF file. If omitted, resolved from subject in data/ "
            "(expects a unique subjXX*.fif match)."
        ),
    )
    parser.add_argument(
        "--trigger-channel",
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Maximum width in samples treated as transient when collapsing "
            "contiguous transition pulses before alignment (default: 1)."
        ),
    )
    parser.add_argument(
        "--out-csv",
        ...
        default=None,
        help=(
            "Output confrontation CSV path. If omitted, defaults to "
            "<project-root>/reports/subXX_blockYY_runZZ_reported_vs_observed.csv."
        ),
    )
    parser.add_argument(
        "--out-report",
        ...
        default=None,
        help=(
            "Output markdown report path. If omitted, defaults to "
            "<project-root>/reports/subXX_blockYY_runZZ_reported_vs_observed.md."
        ),
    )
    parser.add_argument(
        "--anomaly-csv",
    ...
    parser.add_argument(
        ...
        default=3.0,
        help=(
            "Half-window size in ms around each collapsed transition-pulse warning "
            "(default: 3)."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable transition pulse warning plots. Transition warnings are still "
            "computed and reported in the markdown summary."
        ),
    )
    parser.add_argument(
...
def _require_subject_block_run(
...
def _resolve_default_reported_csv(
    project_root: Path,
    *,
    subject: int,
    block: int,
...
def _resolve_default_fif(project_root: Path, *, subject: int) -> Path:
    """
    Resolve the FIF path for a subject from data/ with a unique match.

    Accepted filename prefixes include subjXX and SubjXX.
    """
    data_dir = project_root / "data"
    patterns = [f"subj{subject:02d}*.fif", f"Subj{subject:02d}*.fif"]
    candidates: list[Path] = []
    for pattern in patterns:
        candidates.extend(sorted(data_dir.glob(pattern)))
    # De-duplicate in case a file matches both patterns on case-insensitive FS.
    unique = []
    seen = set()
    ...
    if not unique:
        raise FileNotFoundError(
            f"No FIF file found in {data_dir} for subject {subject:02d}."
        )
    if len(unique) > 1:
        raise ValueError(
            "Multiple FIF files match the subject. Pass --fif explicitly. "
...
def _resolve_default_outputs(
    ...
    *,
    ...
    """Resolve default output CSV/MD paths under reports/."""
    base = (
        project_root
        / "reports"
        / f"sub{subject:02d}_block{block:02d}_run{run:02d}_reported_vs_observed"
    )
...
def main() -> None:
    ...
    if args.reported_csv is None:
        subject, block, run = _require_subject_block_run(
            args.subject, args.block, args.run
        )
        reported_csv = _resolve_default_reported_csv(
            project_root,
            subject=subject,
            block=block,
            run=run,
    ...
    if args.fif is None:
        subject, _, _ = _require_subject_block_run(args.subject, args.block, args.run)
        fif_path = _resolve_default_fif(project_root, subject=subject).resolve()
    else:
        fif_path = args.fif.expanduser().resolve()

    if args.out_csv is None or args.out_report is None:
        if (
            args.subject is not None
            and args.block is not None
            and args.run is not None
        ):
            subject, block, run = _require_subject_block_run(
                args.subject, args.block, args.run
            )
            default_out_csv, default_out_report = _resolve_default_outputs(
                project_root,
                subject=subject,
                block=block,
                run=run,
            ...
            default_base = (
                project_root / "reports" / f"{reported_csv.stem}_reported_vs_observed"
            )
            default_out_csv, default_out_report = (
                default_base.with_suffix(".csv"),
                default_base.with_suffix(".md"),
            )
    else:
        default_out_csv = None
        default_out_report = None

    out_csv = (
        args.out_csv.expanduser().resolve()
        if args.out_csv is not None
        else default_out_csv.resolve()
    )
    out_report = (
        args.out_report.expanduser().resolve()
        if args.out_report is not None
        else default_out_report.resolve()
    )
    anomaly_csv = (
    ...
    transition_plots_dir = plots_dir / "transition_pulses"
    ...
    # Always collapse short contiguous transition pulses and track warnings.
    observed, transition_warnings = collapse_transition_windows(
        observed,
        transient_max_samples=args.transition_max_samples,
    )
    if not observed:
        raise RuntimeError("No observed windows remain after transition-pulse collapsing.")
    ...
    print(
        "WARNINGS: "
        f"{len(transition_warnings)} collapsed short contiguous transition pulses "
        "(0->A->B->0 line-update transients)."
    )
    ...
    if args.disable_plot_transition_pulses:
        print("Transition warning plots disabled by --disable-plot-transition-pulses.")
    else:
        print(f"Saved transition warning plots in: {transition_plots_dir}")
