...
%     unless Catch.CheckNextPositionsBoundaries is set to false.
...
Catch.JitterBase = 0.08; %minimum jitter amplitude in degrees
Catch.JitterScaling = 'log'; %'off', 'linear', 'log', or 'exp' ('on' alias for linear)
% Boundary guard settings for jittered paths.
Catch.CheckNextPositionsBoundaries = false; %false disables the boundary rejection logic
                ...
                if TrialStruct(iRun, iSeq).Congruence(iCatch) == 2
                    ...
                    for iMov = (StartCatch+Catch.OcclDuration * Dat.Cfg.fps):(StartCatch+(Catch.OcclDuration + Catch.OcclVideoDuration)* Dat.Cfg.fps)
                        ...
                        if iMov == StartCatch+Catch.OcclDuration * Dat.Cfg.fps
                            ...
                            % Boundary guard for jittered positions (optional).
                            if Catch.CheckNextPositionsBoundaries
