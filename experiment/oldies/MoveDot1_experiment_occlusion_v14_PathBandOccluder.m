...
%
% Purpose:
%   Run the one-111dot occlusion paradigm with block-aware 3-runs-per-block
%   scheduling and two catch-trial types generated by:
%     CreateInputFiles_v17_threeRun1111111111111111111111111111111111sPerBlock_catch_PathBandOccluder.m
%11111111111111111111111111111111
% Schedule model (from v17 TrialOrder/CatchPlan):
...
%   - Switch source:
%       Config_schedule_CreateInputV17_MoveDotV14_PathBandOccluder.runColorCueEnabled
%       (set true by default in v14 config).
%
% Message flow additions in v14:
%   - Start gate message:
...
%
% Practice mode additions in v14:
...
%   MoveDot1_experiment_occlusion_v14_PathBandOccluder
%
...
%    run('MoveDot1_experiment_occlusion_v14_PathBandOccluder.m');"
%
% Inputs:
%   - experiment/input_files/MovDot_SubXX_V24_PathBandOccluder.mat with
%     occlusion metadata fields.
%   - required:
%       experiment/input_files/SubXX_TrialStruct_v17_threeRunsPerBlock_catch_PathBandOccluder.mat
...
%   - pathBandEntranceStyle       ('round' or 'straight'; optional override
%                                  of schedule config for path-band terminals)
%   - pathBandStraightBackshiftDotRadiusScale
%                                 (non-negative scalar; optional override
%                                  of schedule config when style='straight')
%   - pathBandStraightEndForwardDotRadiusScale
%                                 (non-negative scalar; optional override
%                                  controlling straight-end forward extension
%                                  in dot-radius units)
%   - pathBandStraightPostStartWidthExtraDotRadiusScale
%                                 (non-negative scalar; optional override
%                                  controlling post-occluder entrance width
%                                  boost in dot-radius units)
%   - pathBandStraightStartBackshiftPixelOffset
%                                 (scalar pixel correction applied to straight
%                                  start-wall backshift; negative values move
%                                  first full occlusion later by shrinking the
%                                  computed backshift slightly)
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v14_PathBandOccluder_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v14_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV17_MoveDotV14_PathBandOccluder.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v14_PathBandOccluder';
...
% Start gate and run-transition messages (v14 additions).
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var') && exist('practice_mode', 'var'))
    ...
    dlgtitle = 'Occlusion v14 (practice by run) input';
    dims = [1 50];
...
if ~isfile(trialStructFile)
    error(['Required schedule file not found: %s\n' ...
        'Generate it with CreateInputFiles_v17_threeRunsPerBlock_catch_PathBandOccluder.m first.'], trialStructFile);
end
...
%   Load TrialOrder/CatchPlan from v17 schedule file and resolve selected blocks/runs.
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v14 block mode.', ...
        trialStructFile);
end
...
if isempty(debug_runNumber)
    fprintf('Using v17 TrialOrder from %s (start block %d, runs 1..%d).\n', ...
        trialStructFile, iBlock, runsPerBlockAvailable);
else
    fprintf('Using v17 TrialOrder from %s (start block %d, debug run %d only).\n', ...
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            for iTrial = 1:nTrials
                ...
                % No pre-trial pause in v14 block-break mode:
                ...
                for iFrame = 1:framesPerTrial
                    ...
                    if strcmpi(Conf.occlusionMode, 'alpha')
                        ...
                        if logical(trial.occlusion_enabled) && ~isCatchType1
                            ...
                            if preActive && ~isempty(pathbandPreCoords)
                                local_draw_polyline_segments(window, pathbandPreCoords, ...
                                    pathbandLineWidthPx, Conf.rectColor, pathbandPreDrawOpts);
                            end
                            if postActive && ~isempty(pathbandPostCoords)
                                local_draw_polyline_segments(window, pathbandPostCoords, ...
                                    pathbandLineWidthPx, Conf.rectColor, pathbandPostDrawOpts);
                            end
                        end
                    end
                    ...
                    if logical(Conf.debugShowFullPathOverlay) && ~isempty(pathOverlayCoords)
                        local_draw_polyline_segments(window, pathOverlayCoords, ...
                            Conf.debugPathLineWidthPx, Conf.debugPathLineColor, pathOverlayDrawOpts);
                    end
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v14_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v14 inter-trial sequence.
%
% Sequence:
...
% LOCAL_RESOLVE_BLOCK_RUN_ORDER Return one block/run trial order from v17 TrialOrder.
%
% Inputs:
...
%   Geometry shifts are generated in stimuli_generation_V24_PathBandOccluder
