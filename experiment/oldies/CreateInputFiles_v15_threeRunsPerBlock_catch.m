% CreateInputFiles_v15_threeRunsPerBlock_catch.m
%
% Purpose:
...
%   CreateInputFiles_v15_threeRunsPerBlock_catch
%
% Usage example (non-interactive):
%   /Applications/MATLAB_R2020a.app/bin/matlab -batch ...
%   "cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/experiment'); ...
%    iSub=66; randomSeed=6601; numBlocks=1; ...
%    run('CreateInputFiles_v15_threeRunsPerBlock_catch.m');"
%
% Inputs:
%   - input_files/MovDot_SubXX.mat with:
%       * xySeqs
%       * Cfg
%   - Config_schedule_CreateInputV15_MoveDotV12 constants:
...
%         MoveDot1_experiment_occlusion_v12_practiceByRun.m
%
...
%
% Outputs:
%   - input_files/SubXX_TrialStruct_v15_threeRunsPerBlock_catch.mat with:
%       * TrialStruct  (nRuns x nTrialsFlattened; padded slots allowed)
%       * Catch        (legacy compatibility struct + v15 catch counts)
...
scheduleCfg = Config_schedule_CreateInputV15_MoveDotV12;
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV15_MoveDotV12.runsPerBlock must be 3 for this script.');
end
if abs(scheduleCfg.run2FractionPerOccluded - 0.5) > eps
    error('Config_schedule_CreateInputV15_MoveDotV12.run2FractionPerOccluded must be 0.5.');
end

if ~(exist('iSub', 'var') && exist('randomSeed', 'var'))
    prompt = {'Subject Number:', 'Random seed:'};
    dlgtitle = 'CreateInputFiles v15 (three runs per block + catches)';
    dims = [1 60];
    definput = {'66', '6601'};
...
if baseRunLength ~= 2 * nOccHalf
    error(['Base run length mismatch: run1=%d but run2/run3 base=%d. ' ...
        'Use matched condition counts for v15.'], baseRunLength, 2 * nOccHalf);
end
...
% Data flow: legacy fields + v15 counters -> downstream-compatible container.
Catch = struct();
...
% Explicit v15 catch counters (mirrored to Catch.V14 for legacy readers).
Catch.V15 = struct();
...
Schedule = struct();
Schedule.version = 'v15_three_runs_per_block_with_catches';
...
fprintf('Saved v15 three-runs-per-block catch trial struct: %s\n', outputFile);
