...
%   CreateInputFiles_v19_threeRunsPerBlock_catch_eyeTrackerReplay
%
% Usage example (non-interactive):
%   /Applications/MATLAB_R2020a.app/bin/matlab -batch ...
%   "cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/experiment'); ...
%    iSub=66; randomSeed=6601; numBlocks=1; ...
%    run('CreateInputFiles_v19_threeRunsPerBlock_catch_eyeTrackerReplay.m');"
%
% Inputs:
%   - input_files/MovDot_SubXX_V26_eyeTrackerReplay.mat with:
%       * xySeqs
%       * Cfg
%   - Config_schedule_CreateInputV19_MoveDotV16_eyeTrackerReplay constants:
...
%
% Outputs:
%   - input_files/SubXX_TrialStruct_v19_threeRunsPerBlock_catch_eyeTrackerReplay.mat with:
%       * TrialStruct  (nRuns x nTrialsFlattened; padded slots allowed)
%       * Catch        (legacy compatibility struct + v19 catch counts)
...
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV19_MoveDotV16_eyeTrackerReplay.runsPerBlock must be 3 for this script.');
end
if abs(scheduleCfg.run2FractionPerOccluded - 0.5) > eps
    error('Config_schedule_CreateInputV19_MoveDotV16_eyeTrackerReplay.run2FractionPerOccluded must be 0.5.');
end

if ~(exist('iSub', 'var') && exist('randomSeed', 'var'))
    prompt = {'Subject Number:', 'Random seed:'};
    dlgtitle = 'CreateInputFiles v19 (three runs per block + catches)';
    dims = [1 60];
    definput = {'66', '6601'};
...
if baseRunLength ~= 2 * nOccHalf
    error(['Base run length mismatch: run1=%d but run2/run3 base=%d. ' ...
        'Use matched condition counts for v19.'], baseRunLength, 2 * nOccHalf);
end
...
% Data flow: legacy fields + v19 counters -> downstream-compatible container.
Catch = struct();
...
% Explicit v19 catch counters (mirrored to older fields for legacy readers).
Catch.V19 = struct();
Catch.V19.nCatchType1Total = catchCountType1Total;
Catch.V19.nCatchType2Total = catchCountType2Total;
Catch.V19.nCatchTotal = catchCountType1Total + catchCountType2Total;
Catch.V19.catchRateType1Run1 = double(scheduleCfg.catchRateType1Run1);
Catch.V19.catchRateType2Runs23 = double(scheduleCfg.catchRateType2Runs23);
Catch.V16 = Catch.V19;
Catch.V14 = Catch.V19;
...
Schedule = struct();
Schedule.version = 'v19_three_runs_per_block_with_catches_pathband';
...
fprintf('Saved v19 three-runs-per-block catch trial struct: %s\n', outputFile);
