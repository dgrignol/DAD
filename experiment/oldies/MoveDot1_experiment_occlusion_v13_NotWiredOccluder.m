...
%
% Schedule model (from v16 TrialOrder/CatchPlan):
%   - Run 1: always_visible base  1  trials + inserted type-1 catches.
%   - Run 2: mixed occluded_nondeviant + occluded_deviant base trials +
%           1 inserted type-2 catches.
...
%
% Run-color cue logic:
%   - Optional cue to help distinguish run families:
%       run 1 (always_visible family) vs runs 2/3 (occlusion family).
%   - Colors:
%       * [0 255 0]
%       * [242 223 0]
%   - Counterbalancing by subject parity:
%       odd subject  -> run 1 gets [0 255 0], runs 2/3 get [242 223 0]
%       even subject -> run 1 gets [242 223 0], runs 2/3 get [0 255 0]
%   - Switch source:
%       Config_schedule_CreateInputV16_MoveDotV13_NotWiredOccluder.runColorCueEnabled
%       (set true by default in v13 config).
%
% Message flow additions in v13:
%   - Start gate message:
%       * configurable text/toggle/duration from config
%       * waits for button-box 1/8 or keyboard 1/8 to start
%       * when duration > 0, auto-continues after timeout
%   - Run transition message:
%       * configurable text/toggle/duration from config
%       * shown after run 1 and before later runs
%       * no response required
%
% Practice mode additions in v13:
%   - Selectable at startup dialog via Practice mode (0/1).
%   - Controlled by config value practiceRunPercentageTrials:
%       * keep that percentage of scheduled trials per run
%       * run 1 selected trials become type-1 catches
%       * run 2/3 selected trials become type-2 catches
%       * changed-path catches in run 1 force deviance during disappearance
%   - End-of-session prints catch accuracy summary by run
%     (correct/scored catches with answered/timeout counts).
%
% Usage example (interactive from experiment/):
%   MoveDot1_experiment_occlusion_v13_NotWiredOccluder
%
...
%    run('MoveDot1_experiment_occlusion_v13_NotWiredOccluder.m');"
%
% Inputs:
%   - experiment/input_files/MovDot_SubXX_V23_NotWiredOccluder.mat with occlusion metadata fields.
%   - required:
%       experiment/input_files/SubXX_TrialStruct_v16_threeRunsPerBlock_catch_NotWiredOccluder.mat
...
%   - runColorCueEnabled          (optional runtime override; if omitted,
%                                  Config_schedule_CreateInputV16_MoveDotV13_NotWiredOccluder
%                                  controls the switch)
%   - practice_mode               (0 or 1; optional override of popup field)
%   - practiceRunPercentageTrials (optional override of config percentage)
%   - debugShowFrameInfoOverlay   (0/1; draws trial/frame + elapsed-time
%                                  text in the upper-left corner)
%   - debugShowFullPathOverlay    (0/1; draws the full trial path as a
%                                  persistent line inside the arena)
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v13_practiceByRun_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v13_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
clearvars -except iSub iBlock viewingDistanceMm dryRunValidateScheduleOnly debug_runNumber dryRunSimulatedBreakKeys question_text debugTriggerMode runColorCueEnabled practice_mode practiceRunPercentageTrials debugShowFrameInfoOverlay debugShowFullPathOverlay;
clc;
sca;
format shortG;
addpath('lib/');
...
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV16_MoveDotV13_NotWiredOccluder.runsPerBlock must be 3 for this runtime.');
end
...
Conf.occlusionMode = 'notwired_circle'; % default: notwired_circle; optional: alpha.
Conf.experimentName = 'MoveDot1_occlusion_v13_practiceByRun';
...
% Start gate and run-transition messages (v13 additions).
...
if ~ismember(lower(Conf.occlusionMode), {'notwired_circle', 'alpha'})
    error('Conf.occlusionMode must be ''notwired_circle'' or ''alpha''.');
end
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var') && exist('practice_mode', 'var'))
    ...
    dlgtitle = 'Occlusion v13 (practice by run) input';
    dims = [1 50];
...
if ~isfile(trialStructFile)
    error(['Required schedule file not found: %s\n' ...
        'Generate it with CreateInputFiles_v16_threeRunsPerBlock_catch_NotWiredOccluder.m first.'], trialStructFile);
end
...
requiredFields = { ...
    ...
    'notwired_center_start_xy', 'notwired_radius', ...
    'notwired_hide_start_frame', 'notwired_hide_end_frame', ...
    'notwired_resume_frame', 'notwired_reanchor_enabled', ...
    'notwired_reanchor_center_xy'};
...
%   Load TrialOrder/CatchPlan from v16 schedule file and resolve selected blocks/runs.
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v13 block mode.', ...
        trialStructFile);
end
...
if isempty(debug_runNumber)
    fprintf('Using v16 TrialOrder from %s (start block %d, runs 1..%d).\n', ...
        trialStructFile, iBlock, runsPerBlockAvailable);
else
    fprintf('Using v16 TrialOrder from %s (start block %d, debug run %d only).\n', ...
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            output = repmat(struct( ...
                ...
                'notwired_radius', [], ...
                'notwired_hide_start_frame', [], ...
                'notwired_hide_end_frame', [], ...
                'notwired_resume_frame', [], ...
                'notwired_reanchor_enabled', [], ...
                'notwired_used_fallback_side', [], ...
                'catch_type_code', [], ...
                'catch_type_label', '', ...
                ...
                'debug_frame_info_overlay', false, ...
                'debug_full_path_overlay', false, ...
                'frame_flip_time', [], ...
                'frame_duration_ms', [], ...
                'trial_start_time', [], ...
            ...
            for iTrial = 1:nTrials
                ...
                notwiredRadius = max(1e-6, double(trial.notwired_radius));
                hideStartFrame = occCompleteFrame;
                if isfield(trial, 'notwired_hide_start_frame') && ~isempty(trial.notwired_hide_start_frame)
                    hideStartFrame = local_clamp_frame(double(trial.notwired_hide_start_frame), framesPerTrial);
                end
                hideEndFrame = max(hideStartFrame, occEndStartFrame - 1);
                if isfield(trial, 'notwired_hide_end_frame') && ~isempty(trial.notwired_hide_end_frame)
                    hideEndFrame = local_clamp_frame(double(trial.notwired_hide_end_frame), framesPerTrial);
                end
                hideEndFrame = max(hideStartFrame, hideEndFrame);

                resumeFrame = occEndStartFrame;
                if isfield(trial, 'notwired_resume_frame') && ~isempty(trial.notwired_resume_frame)
                    resumeFrame = local_clamp_frame(double(trial.notwired_resume_frame), framesPerTrial);
                end

                notwiredReanchorEnabled = false;
                if isfield(trial, 'notwired_reanchor_enabled') && ~isempty(trial.notwired_reanchor_enabled)
                    notwiredReanchorEnabled = logical(trial.notwired_reanchor_enabled);
                end
                if ~strcmpi(condLabel, 'occluded_deviant')
                    notwiredReanchorEnabled = false;
                end

                notwiredFallbackSide = false;
                if isfield(trial, 'notwired_used_fallback_side') && ~isempty(trial.notwired_used_fallback_side)
                    notwiredFallbackSide = logical(trial.notwired_used_fallback_side);
                end

                startCenterPx = local_xy_to_px(double(trial.notwired_center_start_xy), ppd, rectCoords);
                reanchorCenterPx = startCenterPx;
                if notwiredReanchorEnabled && isfield(trial, 'notwired_reanchor_center_xy') && ...
                        ~isempty(trial.notwired_reanchor_center_xy)
                    reanchorCenterPx = local_xy_to_px(double(trial.notwired_reanchor_center_xy), ppd, rectCoords);
                end
                notwiredRadiusPx = max(1, notwiredRadius * ppd);
                ...
                if logical(Conf.debugShowFullPathOverlay)
                    ...
                    if nnz(finitePathMask) >= 2
                        pathOverlayCoords = [trialStim(finitePathMask, 1)'; ...
                            trialStim(finitePathMask, 2)'];
                    end
                end
                ...
                output(iTrial).notwired_radius = notwiredRadius;
                output(iTrial).notwired_hide_start_frame = hideStartFrame;
                output(iTrial).notwired_hide_end_frame = hideEndFrame;
                output(iTrial).notwired_resume_frame = resumeFrame;
                output(iTrial).notwired_reanchor_enabled = notwiredReanchorEnabled;
                output(iTrial).notwired_used_fallback_side = notwiredFallbackSide;
                ...
                output(iTrial).debug_frame_info_overlay = logical(Conf.debugShowFrameInfoOverlay);
                output(iTrial).debug_full_path_overlay = logical(Conf.debugShowFullPathOverlay);

                % No pre-trial pause in v13 block-break mode:
                ...
                for iFrame = 1:framesPerTrial
                    ...
                    % NotWired rule:
                    % once fully occluded, do not draw the dot for the full
                    % occlusion interval. Reappearance starts at occEndStartFrame.
                    if logical(trial.occlusion_enabled) && ~isCatchType1 && ...
                            iFrame >= hideStartFrame && iFrame <= hideEndFrame
                        dotVisibleThisFrame = false;
                    end
                    ...
                    if strcmpi(Conf.occlusionMode, 'alpha')
                        ...
                        % notwired_circle mode: draw dot (when enabled), then
                        % render the occluder circle selected for this frame.
                        if dotVisibleThisFrame
                            Screen('DrawDots', window, [xDot; yDot], dotSizePx, runDotColor, [0 0], 1);
                        end

                        % Draw the geometric occluder only from the
                        % configured occlusion start frame onward.
                        if logical(trial.occlusion_enabled) && ~isCatchType1 && ...
                                iFrame >= occStartFrame
                            if notwiredReanchorEnabled && iFrame >= resumeFrame
                                activeCenterPx = reanchorCenterPx;
                            else
                                activeCenterPx = startCenterPx;
                            end
                            ovalRect = [activeCenterPx(1)-notwiredRadiusPx, ...
                                activeCenterPx(2)-notwiredRadiusPx, ...
                                activeCenterPx(1)+notwiredRadiusPx, ...
                                activeCenterPx(2)+notwiredRadiusPx];
                    ...
                    if logical(Conf.debugShowFullPathOverlay) && ~isempty(pathOverlayCoords)
                        Screen('DrawLines', window, pathOverlayCoords, Conf.debugPathLineWidthPx, ...
                            Conf.debugPathLineColor, [0 0], 1);
                    end
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v13_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v13 inter-trial sequence.
%
% Sequence:
...
% LOCAL_RESOLVE_BLOCK_RUN_ORDER Return one block/run trial order from v16 TrialOrder.
%
% Inputs:
...
for iTrial = 1:numel(output)
    if isfield(output(iTrial), 'catch_expected_response_code')
        candidate = double(output(iTrial).catch_expected_response_code);
        if ~isempty(candidate) && isscalar(candidate) && isfinite(candidate)
            expectedVec(iTrial) = candidate;
        end
    end
    if isfield(output(iTrial), 'catch_response_code')
        candidate = double(output(iTrial).catch_response_code);
        if ~isempty(candidate) && isscalar(candidate) && isfinite(candidate)
            responseVec(iTrial) = candidate;
        end
    end
    if isfield(output(iTrial), 'catch_response_correct')
        candidate = double(output(iTrial).catch_response_correct);
        if ~isempty(candidate) && isscalar(candidate) && isfinite(candidate)
            correctVec(iTrial) = candidate;
        end
    end
end
