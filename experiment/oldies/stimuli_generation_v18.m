...
%   addpath('lib');
%   stimuli_generation_v18;
%
% Usage example (non-interactive from repo root):
%   /Applications/MATLAB_R2020a.app/bin/matlab -batch ...
%   "cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/experiment'); addpath('lib'); stimuli_generation_v18;"
%
% Inputs:
...
clearvars -except sourceSubjectID targetSubjectID leadSec lagSec;
clc;
...
if ~(exist('sourceSubjectID', 'var') && exist('targetSubjectID', 'var') && ...
    ...
    dlgTitle = 'v18 occlusion dataset parameters';
    dims = [1 70];
...
for iTrial = 1:nTrials
    ...
    twocircleDev = local_build_twocircle_metadata(trialDev.xy, devFrame, timing, dotRadiusDeg);
    ...
    observedCell{rowOccDev} = local_attach_occlusion_fields( ...
        trialDev, 45, 'occluded_deviant', true, ...
        timing, twocircleDev, alphaOcc, geomOcc);
end
xySeqs = vertcat(observedCell{:});

%% Build predicted output trials (occluded deviant only)
% Data flow: predicted source + observed deviant prefix override -> xySeqsPredicted.
predictedCell = cell(nTrials, 1);
for iTrial = 1:nTrials
    trialObservedDev = observedDeviant(iTrial);
