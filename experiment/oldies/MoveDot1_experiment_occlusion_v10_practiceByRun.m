% MoveDot1_experiment_occlusion_v10_practiceByRun.m
%
% Purpose:
...
%   - Switch source:
%       Config_schedule_CreateInputV15_MoveDotV10.runColorCueEnabled
%       (set true by default in v10 config).
%
% Message flow additions in v10:
%   - Start gate message:
...
%
% Practice mode additions in v10:
...
%   MoveDot1_experiment_occlusion_v10_practiceByRun
%
...
%    run('MoveDot1_experiment_occlusion_v10_practiceByRun.m');"
%
% Inputs:
...
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v10_practiceByRun_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v10_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
clearvars -except iSub iBlock viewingDistanceMm dryRunValidateScheduleOnly debug_runNumber dryRunSimulatedBreakKeys question_text debugTriggerMode runColorCueEnabled practice_mode practiceRunPercentageTrials;
clc;
sca;
format shortG;
addpath('lib/');
...
scheduleCfg = Config_schedule_CreateInputV15_MoveDotV10;
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV15_MoveDotV10.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v10_practiceByRun';
...
% Start gate and run-transition messages (v10 additions).
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var') && exist('practice_mode', 'var'))
    ...
    dlgtitle = 'Occlusion v10 (practice by run) input';
    dims = [1 50];
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v10 block mode.', ...
        trialStructFile);
end
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            for iTrial = 1:nTrials
                ...
                % No pre-trial pause in v10 block-break mode:
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v10_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v10 inter-trial sequence.
%
% Sequence:
...
if round(double(currentRun)) == 1
    ...
    for iKeep = 1:nKeep
        ...
        if branchMask(iKeep)
            altSourceIdx = local_pick_practice_alt_source(trialOrderOut(iKeep), trialOrderOut, xyAll);
            if isfinite(altSourceIdx) && altSourceIdx ~= trialOrderOut(iKeep)
...
msg = sprintf(Conf.endOfBlockMessageTextTemplate, currentBlock, remainingBlocks);
