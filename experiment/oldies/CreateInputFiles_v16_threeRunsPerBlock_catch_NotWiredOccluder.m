% CreateInputFiles_v16_threeRunsPerBlock_catch_NotWiredOccluder.m
%
% Purpose:
...
%   CreateInputFiles_v16_threeRunsPerBlock_catch_NotWiredOccluder
%
% Usage example (non-interactive):
%   /Applications/MATLAB_R2020a.app/bin/matlab -batch ...
%   "cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/experiment'); ...
%    iSub=66; randomSeed=6601; numBlocks=1; ...
%    run('CreateInputFiles_v16_threeRunsPerBlock_catch_NotWiredOccluder.m');"
%
% Inputs:
%   - input_files/MovDot_SubXX_V23_NotWiredOccluder.mat with:
%       * xySeqs
%       * Cfg
%   - Config_schedule_CreateInputV16_MoveDotV13_NotWiredOccluder constants:
...
%         MoveDot1_experiment_occlusion_v13_NotWiredOccluder.m
%
...
%
% Outputs:
%   - input_files/SubXX_TrialStruct_v16_threeRunsPerBlock_catch_NotWiredOccluder.mat with:
%       * TrialStruct  (nRuns x nTrialsFlattened; padded slots allowed)
%       * Catch        (legacy compatibility struct + v16 catch counts)
...
scheduleCfg = Config_schedule_CreateInputV16_MoveDotV13_NotWiredOccluder;
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV16_MoveDotV13_NotWiredOccluder.runsPerBlock must be 3 for this script.');
end
if abs(scheduleCfg.run2FractionPerOccluded - 0.5) > eps
    error('Config_schedule_CreateInputV16_MoveDotV13_NotWiredOccluder.run2FractionPerOccluded must be 0.5.');
end

if ~(exist('iSub', 'var') && exist('randomSeed', 'var'))
    prompt = {'Subject Number:', 'Random seed:'};
    dlgtitle = 'CreateInputFiles v16 (three runs per block + catches)';
    dims = [1 60];
    definput = {'66', '6601'};
...
% Data flow: MovDot_SubXX_V23_NotWiredOccluder.mat -> xySeqs labels -> condition index pools.
...
if baseRunLength ~= 2 * nOccHalf
    error(['Base run length mismatch: run1=%d but run2/run3 base=%d. ' ...
        'Use matched condition counts for v16.'], baseRunLength, 2 * nOccHalf);
end
...
% Data flow: legacy fields + v16 counters -> downstream-compatible container.
Catch = struct();
...
% Explicit v16 catch counters (mirrored to Catch.V14 for legacy readers).
Catch.V15 = struct();
Catch.V15.nCatchType1Total = catchCountType1Total;
Catch.V15.nCatchType2Total = catchCountType2Total;
Catch.V15.nCatchTotal = catchCountType1Total + catchCountType2Total;
Catch.V15.catchRateType1Run1 = double(scheduleCfg.catchRateType1Run1);
Catch.V15.catchRateType2Runs23 = double(scheduleCfg.catchRateType2Runs23);
Catch.V14 = Catch.V15;

%% Build explicit schedule metadata
% Data flow: resolved counts + controls -> Schedule struct for runtime and audit.
Schedule = struct();
Schedule.version = 'v16_three_runs_per_block_with_catches_notwired_occluder';
...
fprintf('Saved v16 three-runs-per-block catch trial struct: %s\n', outputFile);
