% MoveDot1_experiment_occlusion_v9_addedPracticeMode.m
%
% Purpose:
...
%   - Switch source:
%       Config_schedule_CreateInputV15_MoveDotV9.runColorCueEnabled
%       (set true by default in v9 config).
%
% Message flow additions in v9:
%   - Start gate message:
...
%       * force every selected trial to be a catch-question trial
%
% Usage example (interactive from experiment/):
%   MoveDot1_experiment_occlusion_v9_addedPracticeMode
%
...
%    run('MoveDot1_experiment_occlusion_v9_addedPracticeMode.m');"
%
% Inputs:
...
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v9_addedPracticeMode_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v9_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
scheduleCfg = Config_schedule_CreateInputV15_MoveDotV9;
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV15_MoveDotV9.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v9_addedPracticeMode';
...
% Start gate and run-transition messages (v9 additions).
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var') && exist('practice_mode', 'var'))
    ...
    dlgtitle = 'Occlusion v9 (added practice mode) input';
    dims = [1 50];
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v9 block mode.', ...
        trialStructFile);
end
...
if logical(Conf.practiceModeEnabled)
    fprintf(['Practice mode ENABLED: keeping %.2f%% of each run and forcing ' ...
        'all selected trials to catch-question trials.\n'], Conf.practiceRunPercentageTrials);
else
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            [trialOrder, catchTypeVec] = local_apply_practice_mode_to_run( ...
                ...
                Conf.practiceModeEnabled, Conf.practiceRunPercentageTrials);
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            for iTrial = 1:nTrials
                ...
                % No pre-trial pause in v9 block-break mode:
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v9_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v9 inter-trial sequence.
%
% Sequence:
...
function [trialOrderOut, catchTypeOut, catchExpectedOut, catchBranchOut, catchDisappearOut, catchReappearOut, catchAltSourceOut] = ...
        ...
        practiceModeEnabled, practiceRunPercentageTrials)
% LOCAL_APPLY_PRACTICE_MODE_TO_RUN Optionally trim run length and force all-catch trials.
%
% Data flow:
...
% Practice rule: every selected trial is a catch-question trial (type 2).
