...
%
% Schedule model (from v15 TrialOrder/CatchPlan):
...
%   - Switch source:
%       Config_schedule_CreateInputV15_MoveDotV12.runColorCueEnabled
%       (set true by default in v12 config).
%
% Message flow additions in v12:
%   - Start gate message:
...
%
% Practice mode additions in v12:
...
%   MoveDot1_experiment_occlusion_v12_practiceByRun
%
...
%    run('MoveDot1_experiment_occlusion_v12_practiceByRun.m');"
%
% Inputs:
%   - experiment/input_files/MovDot_SubXX.mat with occlusion metadata fields.
%   - required:
%       experiment/input_files/SubXX_TrialStruct_v15_threeRunsPerBlock_catch.mat
...
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v12_practiceByRun_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v12_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV15_MoveDotV12.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v12_practiceByRun';
...
% Start gate and run-transition messages (v12 additions).
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var') && exist('practice_mode', 'var'))
    ...
    dlgtitle = 'Occlusion v12 (practice by run) input';
    dims = [1 50];
...
if ~isfile(trialStructFile)
    error(['Required schedule file not found: %s\n' ...
        'Generate it with CreateInputFiles_v15_threeRunsPerBlock_catch.m first.'], trialStructFile);
end
...
%   Load TrialOrder/CatchPlan from v15 schedule file and resolve selected blocks/runs.
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v12 block mode.', ...
        trialStructFile);
end
...
if isempty(debug_runNumber)
    fprintf('Using v15 TrialOrder from %s (start block %d, runs 1..%d).\n', ...
        trialStructFile, iBlock, runsPerBlockAvailable);
else
    fprintf('Using v15 TrialOrder from %s (start block %d, debug run %d only).\n', ...
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            for iTrial = 1:nTrials
                ...
                % No pre-trial pause in v12 block-break mode:
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v12_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v12 inter-trial sequence.
%
% Sequence:
...
% LOCAL_RESOLVE_BLOCK_RUN_ORDER Return one block/run trial order from v15 TrialOrder.
%
% Inputs:
...
for iTrial = 1:numel(output)
    if isfield(output(iTrial), 'catch_expected_response_code')
        expectedVec(iTrial) = double(output(iTrial).catch_expected_response_code);
    end
    if isfield(output(iTrial), 'catch_response_code')
        responseVec(iTrial) = double(output(iTrial).catch_response_code);
    end
    if isfield(output(iTrial), 'catch_response_correct')
        correctVec(iTrial) = double(output(iTrial).catch_response_correct);
    end
end
