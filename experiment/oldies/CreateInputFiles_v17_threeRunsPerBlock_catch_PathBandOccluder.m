% CreateInputFiles_v17_threeRunsPerBlock_catch_PathBandOccluder.m
%
% Purpose:
...
%   CreateInputFiles_v17_threeRunsPerBlock_catch_PathBandOccluder
%
% Usage example (non-interactive):
%   /Applications/MATLAB_R2020a.app/bin/matlab -batch ...
%   "cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/experiment'); ...
%    iSub=66; randomSeed=6601; numBlocks=1; ...
%    run('CreateInputFiles_v17_threeRunsPerBlock_catch_PathBandOccluder.m');"
%
% Inputs:
%   - input_files/MovDot_SubXX_V24_PathBandOccluder.mat with:
%       * xySeqs
%       * Cfg
%   - Config_schedule_CreateInputV17_MoveDotV14_PathBandOccluder constants:
...
%         MoveDot1_experiment_occlusion_v14_PathBandOccluder.m
%
...
%
% Outputs:
%   - input_files/SubXX_TrialStruct_v17_threeRunsPerBlock_catch_PathBandOccluder.mat with:
%       * TrialStruct  (nRuns x nTrialsFlattened; padded slots allowed)
%       * Catch        (legacy compatibility struct + v17 catch counts)
...
scheduleCfg = Config_schedule_CreateInputV17_MoveDotV14_PathBandOccluder;
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV17_MoveDotV14_PathBandOccluder.runsPerBlock must be 3 for this script.');
end
if abs(scheduleCfg.run2FractionPerOccluded - 0.5) > eps
    error('Config_schedule_CreateInputV17_MoveDotV14_PathBandOccluder.run2FractionPerOccluded must be 0.5.');
end

if ~(exist('iSub', 'var') && exist('randomSeed', 'var'))
    prompt = {'Subject Number:', 'Random seed:'};
    dlgtitle = 'CreateInputFiles v17 (three runs per block + catches)';
    dims = [1 60];
    definput = {'66', '6601'};
...
if baseRunLength ~= 2 * nOccHalf
    error(['Base run length mismatch: run1=%d but run2/run3 base=%d. ' ...
        'Use matched condition counts for v17.'], baseRunLength, 2 * nOccHalf);
end
...
% Data flow: legacy fields + v17 counters -> downstream-compatible container.
Catch = struct();
...
% Explicit v17 catch counters (mirrored to older fields for legacy readers).
Catch.V17 = struct();
Catch.V17.nCatchType1Total = catchCountType1Total;
Catch.V17.nCatchType2Total = catchCountType2Total;
Catch.V17.nCatchTotal = catchCountType1Total + catchCountType2Total;
Catch.V17.catchRateType1Run1 = double(scheduleCfg.catchRateType1Run1);
Catch.V17.catchRateType2Runs23 = double(scheduleCfg.catchRateType2Runs23);
Catch.V15 = Catch.V17;
Catch.V14 = Catch.V17;
...
Schedule = struct();
Schedule.version = 'v17_three_runs_per_block_with_catches_pathband';
...
fprintf('Saved v17 three-runs-per-block catch trial struct: %s\n', outputFile);
