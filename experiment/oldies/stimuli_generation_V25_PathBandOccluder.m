%% Stimuli Generation V25 PathBandOccluder (config-driven one-dot occlusion, no source subject files)
% Script: stimuli_generation_V25_PathBandOccluder.m
% Author: Dami (V25 fixation-collision modes by Codex)
%
% Purpose:
...
%     3) occluded_deviant
%   plus occluded_deviant_predicted in MovDot_SubXX_V25_PathBandOccluder_predicted.mat.
%
% Optional central-fixation collision handling:
%   - configured through Config_stimuli_generation_V25_PathBandOccluder.fixationCollisionMode.
%   - modes:
...
%     Config_stimuli_generation_V25_PathBandOccluder.fixationExclusionRadiusDeg.
%
% Fixed occlusion timeline:
%   - deviance frame fixed at 130 (default, configurable via Config_stimuli_generation_V25_PathBandOccluder)
...
%   - width:
%       * controlled by Config_stimuli_generation_V25_PathBandOccluder.pathBandWidthDeg
%       * constrained to be strictly larger than dot diameter
%   - terminal style:
%       * controlled by Config_stimuli_generation_V25_PathBandOccluder.pathBandTerminalStyle
...
%   addpath('lib');
%   stimuli_generation_V25_PathBandOccluder;
%
...
%   addpath('lib');
%   % In Config_stimuli_generation_V25_PathBandOccluder:
%   %   fixationCollisionMode = 'move'; % or 'retry' / 'off'
%   stimuli_generation_V25_PathBandOccluder;
%
% Inputs:
%   - Config_stimuli_generation_V25_PathBandOccluder class on MATLAB path.
%   - targetSubjectID optional workspace override; otherwise prompted.
%
% Outputs:
%   - input_files/MovDot_SubXX_V25_PathBandOccluder.mat
%       with xySeqs containing always_visible / occluded_nondeviant /
%       occluded_deviant and full occlusion metadata fields.
%   - input_files/MovDot_SubXX_V25_PathBandOccluder_predicted.mat
...
%     Config_stimuli_generation_V25_PathBandOccluder.

addpath('lib/');

%% Setup and runtime parameter defaults
% Data flow: caller overrides + Config_stimuli_generation_V25_PathBandOccluder constants -> validated controls.
...
if ~(exist('fixedDevianceFrame', 'var') && ~isempty(fixedDevianceFrame))
    fixedDevianceFrame = Config_stimuli_generation_V25_PathBandOccluder.fixedDevianceFrame;
end
if ~(exist('fixedOcclusionEndFrame', 'var') && ~isempty(fixedOcclusionEndFrame))
    fixedOcclusionEndFrame = Config_stimuli_generation_V25_PathBandOccluder.fixedOcclusionEndFrame;
end
if ~(exist('overwriteExisting', 'var') && ~isempty(overwriteExisting))
    overwriteExisting = true;
end
if ~(exist('pathBandTerminalStyle', 'var') && ~isempty(pathBandTerminalStyle))
    pathBandTerminalStyle = Config_stimuli_generation_V25_PathBandOccluder.pathBandTerminalStyle;
end
if ~(exist('pathBandStraightBackshiftDotRadiusScale', 'var') && ...
    ...
    pathBandStraightBackshiftDotRadiusScale = ...
        Config_stimuli_generation_V25_PathBandOccluder.pathBandStraightBackshiftDotRadiusScale;
end
...
% Data flow: targetSubjectID + Config_stimuli_generation_V25_PathBandOccluder directories -> output files.
inputDir = Config_stimuli_generation_V25_PathBandOccluder.inputDirectory;
if ~exist(inputDir, 'dir')
    mkdir(inputDir);
end

targetObservedFile = fullfile(inputDir, sprintf( ...
    Config_stimuli_generation_V25_PathBandOccluder.observedFilePattern, targetSubjectID));
targetPredictedFile = fullfile(inputDir, sprintf( ...
    Config_stimuli_generation_V25_PathBandOccluder.predictedFilePattern, targetSubjectID));
...
%% Build generation controls from Config_stimuli_generation_V25_PathBandOccluder
% Data flow: Config_stimuli_generation_V25_PathBandOccluder constants -> trajectory synthesis controls.
stimulusTypeConfig = Config_stimuli_generation_V25_PathBandOccluder.likelihood;
framesPerTrial = round(Config_stimuli_generation_V25_PathBandOccluder.trialDuration * Config_stimuli_generation_V25_PathBandOccluder.frameFrequency);
nTrials = Config_stimuli_generation_V25_PathBandOccluder.trialsPerCondition;
fps = double(Config_stimuli_generation_V25_PathBandOccluder.frameFrequency);
alphaFadeInFrames = max(1, round(0.25 * fps));
dotRadiusDeg = double(Config_stimuli_generation_V25_PathBandOccluder.dotWidth) / 2;
pathBandWidthDeg = double(Config_stimuli_generation_V25_PathBandOccluder.pathBandWidthDeg);
...
if nTrials < 1
    error('Config_stimuli_generation_V25_PathBandOccluder.trialsPerCondition must be >= 1.');
end
if framesPerTrial < 2
...
if ~isfinite(pathBandWidthDeg) || pathBandWidthDeg <= (2 * dotRadiusDeg)
    error(['Config_stimuli_generation_V25_PathBandOccluder.pathBandWidthDeg must be finite and ' ...
...
if ~any(directionVariance == 0)
    error('Config_stimuli_generation_V25_PathBandOccluder.likelihood.directionVariance must include nondeviant value 0.');
end
if ~any(directionVariance > 0)
    error('Config_stimuli_generation_V25_PathBandOccluder.likelihood.directionVariance must include at least one deviant value > 0.');
end
...
if numel(stimulusTypeConfig.pathDuration) > 1
    warning(['Config_stimuli_generation_V25_PathBandOccluder.likelihood.pathDuration has multiple values. ' ...
        'V25 uses the first value for one-dot occlusion generation.']);
end
pathDurationSec = double(stimulusTypeConfig.pathDuration(1));

generationCfg = struct();
generationCfg.framesPerTrial = framesPerTrial;
generationCfg.dotSpeedDegPerFrame = double(Config_stimuli_generation_V25_PathBandOccluder.dotSpeedDegPerFrame);
generationCfg.minPosDeg = [Config_stimuli_generation_V25_PathBandOccluder.dotWidth / 2, Config_stimuli_generation_V25_PathBandOccluder.dotWidth / 2];
generationCfg.maxPosDeg = double(Config_stimuli_generation_V25_PathBandOccluder.dotRectSize) - generationCfg.minPosDeg;
generationCfg.initialCurvatureWindows = local_parse_interval_windows( ...
    Config_stimuli_generation_V25_PathBandOccluder.initialCurvatureWindows, 'Config_stimuli_generation_V25_PathBandOccluder.initialCurvatureWindows', false, -inf, inf);
generationCfg.deviantCurvatureWindows = local_parse_interval_windows( ...
    Config_stimuli_generation_V25_PathBandOccluder.deviantCurvatureWindows, 'Config_stimuli_generation_V25_PathBandOccluder.deviantCurvatureWindows', false, -inf, inf);
generationCfg.deviantTurnWindowsDeg = local_parse_interval_windows( ...
    stimulusTypeConfig.deviantSignedTurnWindows, ...
    'Config_stimuli_generation_V25_PathBandOccluder.likelihood.deviantSignedTurnWindows', true, -180, 180);
generationCfg.flipCurvatureOnDeviant = logical(Config_stimuli_generation_V25_PathBandOccluder.flipCurvatureOnDeviant);
generationCfg.randomizeCurvatureOnDeviant = logical(Config_stimuli_generation_V25_PathBandOccluder.randomizeCurvatureOnDeviant);
...
generationCfg.fixationCollisionMode = lower(strtrim(char( ...
    Config_stimuli_generation_V25_PathBandOccluder.fixationCollisionMode)));
generationCfg.fixationExclusionRadiusDeg = double( ...
    Config_stimuli_generation_V25_PathBandOccluder.fixationExclusionRadiusDeg);
generationCfg.fixationCenterDeg = double(Config_stimuli_generation_V25_PathBandOccluder.dotRectSize(:)') / 2;
generationCfg.fixationMovePaddingDeg = double( ...
    Config_stimuli_generation_V25_PathBandOccluder.fixationMovePaddingDeg);
generationCfg.fixationMoveDirectionSamples = double( ...
    Config_stimuli_generation_V25_PathBandOccluder.fixationMoveDirectionSamples);
generationCfg.fixationMoveShiftSamples = double( ...
    Config_stimuli_generation_V25_PathBandOccluder.fixationMoveShiftSamples);

% Validate fixation-collision controls once, then enforce them per trial.
if numel(generationCfg.fixationCenterDeg) ~= 2
    error('Config_stimuli_generation_V25_PathBandOccluder.dotRectSize must be [width height].');
end
if ~ismember(generationCfg.fixationCollisionMode, {'off', 'retry', 'move'})
    error(['Config_stimuli_generation_V25_PathBandOccluder.fixationCollisionMode must be one of: ' ...
        '''off'', ''retry'', ''move''.']);
end
if ~isfinite(generationCfg.fixationExclusionRadiusDeg) || generationCfg.fixationExclusionRadiusDeg < 0
    error('Config_stimuli_generation_V25_PathBandOccluder.fixationExclusionRadiusDeg must be finite and >= 0.');
end
if ~isfinite(generationCfg.fixationMovePaddingDeg) || generationCfg.fixationMovePaddingDeg < 0
    error('Config_stimuli_generation_V25_PathBandOccluder.fixationMovePaddingDeg must be finite and >= 0.');
end
if generationCfg.fixationMoveDirectionSamples < 4 || ...
    ...
    error('Config_stimuli_generation_V25_PathBandOccluder.fixationMoveDirectionSamples must be an integer >= 4.');
end
if generationCfg.fixationMoveShiftSamples < 10 || ...
    ...
    error('Config_stimuli_generation_V25_PathBandOccluder.fixationMoveShiftSamples must be an integer >= 10.');
end
...
maxTurnRadiusDeg = min(double(Config_stimuli_generation_V25_PathBandOccluder.dotRectSize) - double(Config_stimuli_generation_V25_PathBandOccluder.dotWidth)) / 2;
if maxTurnRadiusDeg <= 0
    error('Config_stimuli_generation_V25_PathBandOccluder.dotRectSize minus dotWidth must be > 0.');
end
...
for iTrial = 1:nTrials
    ...
    % Modeling assumption retained from v18-v25:
...
% Data flow: Config_stimuli_generation_V25_PathBandOccluder snapshot + runtime values -> Cfg + repro outputs.
Cfg = struct();
Cfg.dpf = Config_stimuli_generation_V25_PathBandOccluder.dotSpeedDegPerFrame;
Cfg.fps = Config_stimuli_generation_V25_PathBandOccluder.frameFrequency;
Cfg.Stimulitype = Config_stimuli_generation_V25_PathBandOccluder.stimulusType;
Cfg.dot_w = Config_stimuli_generation_V25_PathBandOccluder.dotWidth;
Cfg.rectSize = Config_stimuli_generation_V25_PathBandOccluder.dotRectSize;
Cfg.DirChange = stimulusTypeConfig.directionChange;

configProps = properties('Config_stimuli_generation_V25_PathBandOccluder');
configSnapshot = struct();
for iProp = 1:numel(configProps)
    propName = configProps{iProp};
    configSnapshot.(propName) = Config_stimuli_generation_V25_PathBandOccluder.(propName);
end

repro = struct();
repro.script = struct( ...
    'name', mfilename, ...
    'version', 'V25_pathBandOccluder', ...
    'parameters', struct( ...
...
repro.v25_pathband_occlusion = struct( ...
    'script', mfilename, ...
...
%% Save outputs
% Data flow: generated structs -> MAT files in Config_stimuli_generation_V25_PathBandOccluder.inputDirectory.
...
error(['Failed to generate a feasible trial after %d attempts (sequence %d). ' ...
    'Check Config_stimuli_generation_V25_PathBandOccluder geometry and curvature settings.'], ...
...
% LOCAL_BUILD_OCCLUSION_TIMING Build fixed-frame V25 occlusion event indices.
