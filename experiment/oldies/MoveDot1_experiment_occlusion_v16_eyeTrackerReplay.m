% MoveDot1_experiment_occlusion_v16_eyeTrackerReplay.m
%
% Purpose:
%   Run the one-dot occlusion paradigm with three runs per block and
%   catch-trial logic planned upstream by:
%     CreateInputFiles_v19_threeRunsPerBlock_catch_eyeTrackerReplay.m
%
% Schedule model (from v19 TrialOrder/CatchPlan):
...
%   MoveDot1_experiment_occlusion_v16_eyeTrackerReplay
%
...
%    run('MoveDot1_experiment_occlusion_v16_eyeTrackerReplay.m');"
%
% Inputs:
%   - experiment/input_files/MovDot_SubXX_V26_eyeTrackerReplay.mat
%   - experiment/input_files/SubXX_TrialStruct_v19_threeRunsPerBlock_catch_eyeTrackerReplay.mat
%
...
%   - eyeTrackerDoCalibration, eyeTrackerSendTriggerMessages
%   - eyeTrackerImageTransferEnabled
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v16_eyeTrackerReplay_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v16_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
clearvars -except iSub iBlock viewingDistanceMm dryRunValidateScheduleOnly debug_runNumber dryRunSimulatedBreakKeys question_text debugTriggerMode runColorCueEnabled practice_mode practiceRunPercentageTrials debugShowFrameInfoOverlay debugShowFullPathOverlay pathBandEntranceStyle pathBandStraightBackshiftDotRadiusScale pathBandStraightEndForwardDotRadiusScale pathBandStraightPostStartWidthExtraDotRadiusScale pathBandStraightStartBackshiftPixelOffset debugSkipSyncTests flipMissedWarnThresholdCount flipMissedAbortEnabled flipMissedAbortThresholdCount trackeye ignoreEyeTracker fakeEyeTracker fakeGazeBreakRate enableFixationAbort fixWindowDeg fixBreakToleranceFrames fixationWarningDurationSec replayLimit allowInfiniteReplays eyeTrackerDoCalibration eyeTrackerSendTriggerMessages eyeTrackerImageTransferEnabled;
clc;
sca;
format shortG;
addpath('lib/');
...
scheduleCfg = Config_schedule_CreateInputV19_MoveDotV16_eyeTrackerReplay;
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV19_MoveDotV16_eyeTrackerReplay.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v16_eyeTrackerReplay';
...
Conf.eyeTrackerReceiveFileAfterRun = logical(local_schedule_get_or_default(scheduleCfg, 'eyeTrackerReceiveFileAfterRun', true));
...
% Start gate and run-transition messages (retained in v16).
...
% Timing diagnostics controls (v16):
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var') && exist('practice_mode', 'var'))
    ...
    dlgtitle = 'Occlusion v16 (practice by run) input';
    dims = [1 50];
...
if ~isfile(trialStructFile)
    error(['Required schedule file not found: %s\n' ...
        'Generate it with CreateInputFiles_v19_threeRunsPerBlock_catch_eyeTrackerReplay.m first.'], trialStructFile);
end
...
%   Load TrialOrder/CatchPlan from v19 schedule file and resolve selected blocks/runs.
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v16 block mode.', ...
        trialStructFile);
end
...
if isempty(debug_runNumber)
    fprintf('Using v19 TrialOrder from %s (start block %d, runs 1..%d).\n', ...
        trialStructFile, iBlock, runsPerBlockAvailable);
else
    fprintf('Using v19 TrialOrder from %s (start block %d, debug run %d only).\n', ...
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            while iTrial <= numel(trialSchedule)
                ...
                % No pre-trial pause in v16 block-break mode:
                ...
                if isReplayTrial && ~replayStartTriggerSent
                    [debugTriggerLog, debugTriggerTrial, debugTriggerFrame, debugTriggerLabel] = ...
                        ...
                        'replay_start', debugTriggerLog, debugTriggerTrial, debugTriggerFrame, debugTriggerLabel);
                        ...
                        if hasSample
                            ...
                            if gazeOutsideCounter >= Conf.fixBreakToleranceFrames
                                ...
                                [debugTriggerLog, debugTriggerTrial, debugTriggerFrame, debugTriggerLabel] = ...
                                    ...
                                    'gaze_break', debugTriggerLog, debugTriggerTrial, debugTriggerFrame, debugTriggerLabel);
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v16_sub%02d_block%02d_run%02d.csv', ...
...
baseName = local_make_eyelink_edf_basename(iSub, iBlock);
Conf.eyeTrackerEdfFile = [baseName '.edf'];
Conf.eyeTrackerLocalEdfPath = fullfile(outputDir, Conf.eyeTrackerEdfFile);
Eyelink('Openfile', Conf.eyeTrackerEdfFile);
Eyelink('command', sprintf(['add_file_preamble_text ' ...
    '''MoveDot occlusion v16: subject %d ; block %d ; practice %d ; time %s'''], ...
    iSub, iBlock, double(Conf.practiceModeEnabled), datestr(now, 'YYYYmmddHHMMSS')));
...
function baseName = local_make_eyelink_edf_basename(iSub, iBlock)
% LOCAL_MAKE_EYELINK_EDF_BASENAME Build <=8-char EDF basename.
% Format: MDssbbtt, where ss=subject mod 100, bb=block mod 100, tt=timestamp mod 100.
subjectPart = mod(round(double(iSub)), 100);
blockPart = mod(round(double(iBlock)), 100);
timePart = mod(floor(now * 24 * 60), 100);
baseName = sprintf('MD%02d%02d%02d', subjectPart, blockPart, timePart);
end
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v16 inter-trial sequence.
%
% Sequence:
...
% LOCAL_RESOLVE_BLOCK_RUN_ORDER Return one block/run trial order from v19 TrialOrder.
%
% Inputs:
...
%   Geometry shifts are generated in stimuli_generation_V26_eyeTrackerReplay
...
if eyelinkSessionActive
    ...
    % Retrieve EDF only for real EyeLink acquisition mode.
    if isfield(Conf, 'useEyelink') && Conf.useEyelink && ...
            isfield(Conf, 'eyeTrackerReceiveFileAfterRun') && Conf.eyeTrackerReceiveFileAfterRun && ...
