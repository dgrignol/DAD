% MoveDot1_experiment_occlusion_v7_runColorCueMessages.m
%
% Purpose:
...
%   - Switch source:
%       Config_schedule_CreateInputV14_MoveDotV7.runColorCueEnabled
%       (set true by default in v7 config).
%
% Message flow additions in v7:
%   - Start gate message:
...
%   MoveDot1_experiment_occlusion_v7_runColorCueMessages
%
...
%    run('MoveDot1_experiment_occlusion_v7_runColorCueMessages.m');"
%
% Inputs:
...
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v7_runColorCueMessages_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v7_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
clearvars -except iSub iBlock viewingDistanceMm dryRunValidateScheduleOnly debug_runNumber dryRunSimulatedBreakKeys question_text debugTriggerMode runColorCueEnabled;
clc;
sca;
format shortG;
addpath('lib/');
...
scheduleCfg = Config_schedule_CreateInputV14_MoveDotV7;
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV14_MoveDotV7.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v7_runColorCueMessages';
...
% Start gate and run-transition messages (v7 additions).
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var'))
    ...
    dlgtitle = 'Occlusion v7 (run color cue + messages) input';
    dims = [1 50];
    definput = {'70', '1', '1000'};
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v7 block mode.', ...
        trialStructFile);
end
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            for iTrial = 1:nTrials
                ...
                % No pre-trial pause in v7 block-break mode:
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v7_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v7 inter-trial sequence.
%
% Sequence:
...
while true
    ...
    [accepted, escapePressed] = local_poll_start_message_response(Conf);
    if escapePressed
        action = 'abort';
        return;
    end
...
function [accepted, escapePressed] = local_poll_start_message_response(Conf)
...
if Conf.enableKeyboardDebugResponse
    ...
    if keyDown
        ...
        if any(keyCode(Conf.startMessageAcceptKeyCodes))
            accepted = true;
            KbReleaseWait;
            return;
        end
...
if Conf.MEG
    ...
    if status.newLogFrames > 0 && mod(status.currentWriteFrame, 2)
        ...
        if ~isempty(data)
            rawValue = bitand(uint32(data(end)), Conf.megButtonMask);
            if any(rawValue == Conf.startMessageAcceptMegValues)
                accepted = true;
                return;
            end
        end
