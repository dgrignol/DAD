% MoveDot1_experiment_occlusion_v8_runColorCueMessages.m
%
% Purpose:
...
%   - New per-trial  1sequence-identity trigger:
...
%   - Switch source:
%       Config_schedule_CreateInputV14_MoveDotV8.runColorCueEnabled
%       (set true by default in v8 config).
%
% Message flow additions in v8:
%   - Start gate message:
...
%   MoveDot1_experiment_occlusion_v8_runColorCueMessages
%
...
%    run('MoveDot1_experiment_occlusion_v8_runColorCueMessages.m');"
%
% Inputs:
...
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v8_runColorCueMessages_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v8_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV14_MoveDotV8.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v8_runColorCueMessages';
...
% Start gate and run-transition messages (v8 additions).
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var'))
    ...
    dlgtitle = 'Occlusion v8 (run color cue + messages) input';
    dims = [1 50];
    definput = {'70', '1', '1000'};
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v8 block mode.', ...
        trialStructFile);
end
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            for iTrial = 1:nTrials
                ...
                % No pre-trial pause in v8 block-break mode:
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v8_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v8 inter-trial sequence.
%
% Sequence:
