...
%   stimuli_generation_V28_rescueTraject;
%
% Usage example (generate for a display-specific refresh rate):
%   addpath('lib');
%   frameFrequencyOverride = 60;
%   targetSubjectID = 91;
%   stimuli_generation_V28_rescueTraject;
%
...
%   - frameFrequencyOverride optional workspace override (Hz). When set,
%     generation uses this frame rate and writes matching Cfg.fps/dpf.
%
% Outputs:
...
clearvars -except overwriteExisting fixedDevianceFrame fixedOcclusionEndFrame targetSubjectID pathBandTerminalStyle pathBandStraightBackshiftDotRadiusScale enableTurnMagnitudeRescue turnRescueScaleGrid frameFrequencyOverride;
clc;
...
if ~(exist('frameFrequencyOverride', 'var') && ~isempty(frameFrequencyOverride))
    generationFrameFrequencyHz = double(Config_stimuli_generation_V28_rescueTraject.frameFrequency);
else
    generationFrameFrequencyHz = double(frameFrequencyOverride);
end
...
if ~isscalar(generationFrameFrequencyHz) || ~isfinite(generationFrameFrequencyHz) || ...
        generationFrameFrequencyHz <= 0
    error('frameFrequencyOverride (or config frameFrequency) must be a finite scalar > 0.');
end
generationFrameFrequencyHz = round(generationFrameFrequencyHz);
if generationFrameFrequencyHz < 1
    error('Resolved generation frame frequency must be >= 1 Hz.');
end
...
framesPerTrial = round(Config_stimuli_generation_V28_rescueTraject.trialDuration * generationFrameFrequencyHz);
nTrials = Config_stimuli_generation_V28_rescueTraject.trialsPerCondition;
fps = double(generationFrameFrequencyHz);
...
generationCfg.dotSpeedDegPerFrame = ...
    double(Config_stimuli_generation_V28_rescueTraject.dotSpeedDegPerSec) / fps;
...
Cfg = struct();
Cfg.dpf = generationCfg.dotSpeedDegPerFrame;
Cfg.fps = generationFrameFrequencyHz;
...
repro.script = struct( ...
    ...
    'parameters', struct( ...
        ...
        'generationFrameFrequencyHz', generationFrameFrequencyHz, ...
...
fprintf('Generation frame frequency: %d Hz.\n', generationFrameFrequencyHz);
