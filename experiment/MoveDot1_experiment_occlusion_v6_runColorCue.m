% MoveDot1_experiment_occlusion_v6_runColorCue.m
%
% Purpose:
...
%   - Switch source:
%       Config_schedule_CreateInputV14_MoveDotV6.runColorCueEnabled
%       (set true by default in v6 config).
%
% Usage example (interactive from experiment/):
%   MoveDot1_experiment_occlusion_v6_runColorCue
%
...
%    run('MoveDot1_experiment_occlusion_v6_runColorCue.m');"
%
% Inputs:
...
%
% Outputs:
%   - experiment/output_files/MoveDot1_occlusion_v6_runColorCue_SUBXX_BLOCKYY.mat
%   - experiment/output_files/debug_actual_triggers_occlusion_v6_subXX_blockYY_runZZ.csv (debug mode)
%
% Assumptions:
...
if scheduleCfg.runsPerBlock ~= 3
    error('Config_schedule_CreateInputV14_MoveDotV6.runsPerBlock must be 3 for this runtime.');
end
...
Conf.experimentName = 'MoveDot1_occlusion_v6_runColorCue';
...
if ~(exist('iSub', 'var') && exist('iBlock', 'var') && exist('viewingDistanceMm', 'var'))
    ...
    dlgtitle = 'Occlusion v6 (run color cue + sequence triggers) input';
    dims = [1 50];
    definput = {'70', '1', '1000'};
...
if ndims(tsData.TrialOrder) ~= 3
    error('TrialOrder in %s must be 3D [block x trial x run] for v6 block mode.', ...
        trialStructFile);
end
    ...
    for iBlockIdx = 1:numel(blocksToExecute)
        ...
        for iRunIdx = 1:numel(runsToExecute)
            ...
            for iTrial = 1:nTrials
                ...
                % No pre-trial pause in v6 block-break mode:
            ...
            if Conf.debug || ~isempty(debugTriggerLog)
                debugTriggerFile = fullfile(outputDir, sprintf( ...
                    'debug_actual_triggers_occlusion_v6_sub%02d_block%02d_run%02d.csv', ...
...
% LOCAL_PRESENT_POST_TRIAL_INTERVAL Present the v6 inter-trial sequence.
%
% Sequence:
