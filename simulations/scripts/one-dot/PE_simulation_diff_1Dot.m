% PE_simulation_diff_1Dot.m
%
% Purpose:
%   Run the PE dRSA pipeline with the same structure as
%   simulations/scripts/PE_simulation_diff.m, but strictly for one-dot
%   inputs generated by experiment/stimuli_generation_v17.m.
%
...
%   - Keep standard nondeviant/deviant outputs.
...
%   3) Load one-dot predicted deviant trials from MovDot_SubXX_predicted.mat.
%   4) Run standard nondeviant/deviant dRSA.
...
%   PE_simulation_diff_1Dot;
%
%   % Example 2: run from anywhere with explicit participant + corr.
%   participantNumber = 73;
%   dRSAtypeToRun = 'corr';
%   run('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/simulations/scripts/one-dot/PE_simulation_diff_1Dot.m');
%
...
%   % "cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD'); participantNumber=73; dRSAtypeToRun='corr'; run('simulations/scripts/one-dot/PE_simulation_diff_1Dot.m');"
%
% Inputs:
...
%       simulations/output/SubXX_oneDot/<condition>/<dRSAtype>/{results,diagonal,matrices}
...
%   - PE matrices:
%       simulations/output/SubXX_oneDot/deviant/<dRSAtype>/matrices/PE/commCbar/*.png
%       simulations/output/SubXX_oneDot/deviant/<dRSAtype>/matrices/PE/sepCbar/*.png
%   - PE paths figure:
%       simulations/output/SubXX_oneDot/paths/*PE_paths*.png
%   - Results MAT fields:
%       dRSA_PE.neuralPE
...
%
% Assumptions:
%   - Input uses one-dot xy columns [x y] from stimuli_generation_v17.m.
...
% Data flow: raw MovDot_SubXX.mat -> one-dot center-relative arrays -> 1x2 path figure.
...
for iCond = 1:numel(allConditions)
    ...
    [condDot1Trials, ~] = local_load_one_dot_condition_trials( ...
        inputFile, condName, centerRelativeRectSize, false);
    if cutPostDevPlot
        % Plot-only gate: keep plotting behavior independent from analysis trimming.
        [condDot1Trials, ~] = local_cut_postdeviant(condDot1Trials, deviantOnset);
    end
...
for iCond = 1:numel(inputConditions)
    ...
    subjectOutputDir = fullfile(outputDir, sprintf('Sub%02d_oneDot', participantNumber));
    if cutPostDev
    ...
    if reuseExistingResults
        ...
        %% Load predicted one-dot deviant streams (deviant only)
        % Data flow: MovDot_SubXX_predicted.mat -> deviant condition -> one-dot arrays.
        dot1TrialsPredicted = [];
        predictedSimulationFile = '';
        predictedInputField = '';
        predictedInputColumns = [];
        if usePredictedModels
            ...
            [dot1TrialsPredicted, predictedLoadMeta] = local_load_one_dot_condition_trials( ...
                predictedInputFile, 'deviant', centerRelativeRectSize, true);
            ...
            if ~isequal(size(dot1TrialsPredicted), size(dot1Trials))
                error(['Predicted deviant one-dot trials do not match observed deviant size. ' ...
        ...
        if usePredictedModels
            ...
            if ~peObservedPredictedSizeMatchBeforeCut
                error('Observed and predicted one-dot streams must match before PE post-deviance cut.');
            end
            ...
            if ~peObservedPredictedSizeMatchAfterCut
                error('Observed and predicted one-dot streams diverged after PE post-deviance cut.');
            end
...
%       'experiment/input_files/MovDot_Sub73.mat', 'nondeviant', [], false);
%   [predDev, meta] = local_load_one_dot_condition_trials( ...
%       'experiment/input_files/MovDot_Sub73_predicted.mat', 'deviant', [], true);
%
% Inputs:
...
if ~isfield(allTrials, 'condition')
    error('Condition field missing in %s. Cannot split nondeviant/deviant.', rawInputFile);
end
conditionLabel = lower(char(conditionLabel));
condValues = arrayfun(@(s) s.condition, allTrials);

% Canonical split used by trigger/task logic:
% condition == 0 => nondeviant, condition > 0 (e.g., 45) => deviant.
...
function figHandle = local_plot_all_condition_paths_onedot( ...
        nondeviantPaths, deviantPaths, sampleCount, sampleRateHz, titleText, figVisibility)
% LOCAL_PLOT_ALL_CONDITION_PATHS_ONEDOT Plot sampled one-dot paths for both conditions.
%
% Inputs:
%   nondeviantPaths : trials x 2 x time one-dot nondeviant paths.
%   deviantPaths    : trials x 2 x time one-dot deviant paths.
%   sampleCount     : max number of trials sampled per condition.
...
%
% Data flow:
%   full condition arrays -> sampled trials -> two side-by-side path panels.

nondeviantSample = local_sample_paths_for_plot(nondeviantPaths, sampleCount);
deviantSample = local_sample_paths_for_plot(deviantPaths, sampleCount);

figHandle = figure('Name', char(titleText), 'NumberTitle', 'off', 'Visible', figVisibility);
tiledlayout(1, 2, 'Padding', 'compact', 'TileSpacing', 'compact');
ax1 = nexttile(1);
plot_paths(nondeviantSample, 'ParentAxes', ax1, 'SavePlot', false, ...
    'Title', 'Nondeviant: dot1', 'SampleRateHz', sampleRateHz, ...
    'AxisLimits', []);
ax2 = nexttile(2);
plot_paths(deviantSample, 'ParentAxes', ax2, 'SavePlot', false, ...
    'Title', 'Deviant: dot1', 'SampleRateHz', sampleRateHz, ...
    'AxisLimits', []);
sgtitle(figHandle, char(titleText));
end

function sampledPaths = local_sample_paths_for_plot(paths, sampleCount)
% LOCAL_SAMPLE_PATHS_FOR_PLOT Sample up to sampleCount trials for path visualization.
if isempty(paths)
    sampledPaths = paths;
    return;
end
...
drawIdx = randperm(nTrials, nPick);
