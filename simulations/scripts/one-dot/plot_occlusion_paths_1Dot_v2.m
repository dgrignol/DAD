% plot_occlusion_paths_1Dot_v2.m
%
% Purpose:
...
%     - occluded_deviant
%
%   from a version-aware observed file resolver in experiment/input_files/.
%   Resolver priority:
%     1) MovDot_SubXX_V27_eyeTrackerReplay_blockResume.mat
%     2) MovDot_SubXX.mat
%     3) highest-version MovDot_SubXX*.mat candidate (excluding predicted)
%
%   Then plots the same sampled trial identities across all three
%   conditions using shared x/y limits.
%
% Usage example (interactive):
%   cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/simulations/scripts/one-dot');
%   plot_occlusion_paths_1Dot_v2
%
% Usage example (non-interactive):
%   /Applications/MATLAB_R2020a.app/bin/matlab -batch ...
%   "cd('/Users/damiano/Documents/UniTn/Dynamo/Attention/DAD/simulations/scripts/one-dot'); participantNumber=70; numTrialsToPlot=12; randomSeed=7; run('plot_occlusion_paths_1Dot_v2.m');"
%
...
%
% Key assumptions:
%   - Resolved observed MovDot_SubXX*.mat contains one-dot trajectories
%     (xy columns [x y]).
...
inputFile = local_resolve_observed_input_file(repoRoot, participantNumber);
if ~isfile(inputFile)
...
function observedFile = local_resolve_observed_input_file(repoRoot, participantNumber)
% LOCAL_RESOLVE_OBSERVED_INPUT_FILE Resolve observed MovDot input across naming versions.
%
% Inputs:
%   repoRoot          : repository root path.
%   participantNumber : participant ID used in MovDot_SubXX patterns.
%
% Output:
%   observedFile : chosen observed file path.

inputDir = fullfile(repoRoot, 'experiment', 'input_files');
if ~isfolder(inputDir)
    error('Input directory not found: %s', inputDir);
end

preferredV27 = fullfile(inputDir, sprintf('MovDot_Sub%02d_V27_eyeTrackerReplay_blockResume.mat', participantNumber));
preferredLegacy = fullfile(inputDir, sprintf('MovDot_Sub%02d.mat', participantNumber));
allCandidates = dir(fullfile(inputDir, sprintf('MovDot_Sub%02d*.mat', participantNumber)));

candidateList = {preferredV27, preferredLegacy};
for i = 1:numel(allCandidates)
    curr = fullfile(inputDir, allCandidates(i).name);
    if endsWith(curr, '_predicted.mat')
        continue;
    end
    candidateList{end + 1} = curr; %#ok<AGROW>
end

[observedFile, rankedTable] = local_pick_best_observed_file( ...
    candidateList, preferredV27, preferredLegacy);
if isempty(observedFile)
    error(['No observed input found for participant %02d in %s. Expected one of:\n' ...
        '  %s\n  %s\n  or another MovDot_Sub%02d*.mat candidate.'], ...
        participantNumber, inputDir, preferredV27, preferredLegacy, participantNumber);
end

fprintf('Resolved observed input file: %s\n', observedFile);
if size(rankedTable, 1) > 1
    fprintf('Other compatible observed candidates were found and ranked lower.\n');
end
end

function [bestFile, rankedTable] = local_pick_best_observed_file( ...
        candidateList, preferredV27, preferredLegacy)
% LOCAL_PICK_BEST_OBSERVED_FILE Rank and choose best existing observed candidate.
%
% Ranking policy:
%   1) explicit V27 block-resume filename.
%   2) explicit legacy filename.
%   3) remaining existing candidates by parsed version (higher is newer).

uniqueCandidates = unique(candidateList, 'stable');
scoreRows = [];
for i = 1:numel(uniqueCandidates)
    curr = char(uniqueCandidates{i});
    if ~isfile(curr)
        continue;
    end
    [~, baseName, ext] = fileparts(curr);
    fileName = [baseName, ext];
    if endsWith(fileName, '_predicted.mat')
        continue;
    end
    if strcmp(curr, preferredV27)
        fileScore = 2000;
    elseif strcmp(curr, preferredLegacy)
        fileScore = 1500;
    else
        parsedVersion = local_extract_version_from_filename(fileName);
        if isfinite(parsedVersion)
            fileScore = 1000 + parsedVersion;
        else
            fileScore = 500;
        end
        if contains(lower(fileName), 'blockresume')
            fileScore = fileScore + 20;
        end
    end
    scoreRows = [scoreRows; fileScore, i]; %#ok<AGROW>
end

bestFile = '';
rankedTable = cell(0, 2);
if isempty(scoreRows)
    return;
end
[~, orderIdx] = sortrows(scoreRows, [-1, 2]);
for k = 1:numel(orderIdx)
    row = scoreRows(orderIdx(k), :);
    score = row(1);
    listIdx = row(2);
    rankedTable{k, 1} = char(uniqueCandidates{listIdx}); %#ok<AGROW>
    rankedTable{k, 2} = score; %#ok<AGROW>
end
bestFile = rankedTable{1, 1};
end

function versionValue = local_extract_version_from_filename(fileName)
% LOCAL_EXTRACT_VERSION_FROM_FILENAME Parse trailing experiment version Vxx from filenames.
versionValue = nan;
token = regexp(fileName, '[vV](\d+)', 'tokens', 'once');
if ~isempty(token)
    versionValue = str2double(token{1});
end
end
